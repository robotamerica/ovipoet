...
# A Fork of [OVIWrite](https://github.com/MiragianCycle/OVIWrite)

I have adapted OviWrite for poets, creating 'ovipoet', an Integrated Poetry Environment (IPW): This build includes a poetry mode (:PoetryMode) where each line and stanza can be separated by "|" and tab="/quad{}" which are read and displayed by pandoc.

## Using ovioet with Pandoc: 

- Write your poems in Markdown under the poems/ folder, using Pandoc’s line blocks (| line text) for poetry line breaks and \quad{} for indentation
- The helper scripts (scripts/build.sh and scripts/watch.sh) assemble all content from frontmatter/, poems/, and backmatter/ into build/manuscript.md
